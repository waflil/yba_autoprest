...
if not LocalPlayer.PlayerGui:FindFirstChild("HUD") then
    print("HUD не найден — ждём 10 сек...")
    for i = 1, 20 do
        task.wait(0.5)
        if LocalPlayer.PlayerGui:FindFirstChild("HUD") then break end
    end
    if not LocalPlayer.PlayerGui:FindFirstChild("HUD") then
        warn("HUD не появился — возможно, игра не загрузилась или ты не в YBA")
        return
    end
end

print("HUD найден! Продолжаем...")

-- Нажимаем Play (если нужно)
if Character and Character:FindFirstChild("RemoteEvent") then
    Character.RemoteEvent:FireServer("PressedPlay")
end

-- Удаляем экраны загрузки
for _, gui in ipairs(LocalPlayer.PlayerGui:GetChildren()) do
    if gui.Name:match("LoadingScreen") then
        gui:Destroy()
    end
end

-- Отключаем хуки — чтобы не было ошибок в Xeno
--[[
local itemHook;
...
-- === СТАРТ ===
print("Всё готово — запускаем автоисторию...")
task.spawn(autoStory)
...
repeat task.wait(0.5) until LocalPlayer.PlayerStats and LocalPlayer.PlayerStats.Level
print("Всё готово — запускаем автоисторию...")
task.spawn(autoStory)
