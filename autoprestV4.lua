...
-- === ЗАПУСК СЮЖЕТА (если квестов нет) ===
local Quests = LocalPlayer.PlayerGui.HUD.Main.Frames.Quest.Quests
if #Quests:GetChildren() == 0 then
    print("Нет квестов — запускаю сюжет вручную...")
    
    -- Подходим к Giorno
    local giorno = workspace.Dialogues:FindFirstChild("Giorno")
    if giorno and giorno:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.CFrame = giorno.HumanoidRootPart.CFrame * CFrame.new(0, 0, 5)
        wait(1)
